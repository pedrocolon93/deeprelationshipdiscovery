...
#      \"retrogan_$PERCENTAGEREP\" \"oov_test_$PERCENTAGEREP/retrogan_$PERCENTAGEREP/\""
     CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES $PATH_TO_RETROGAN_PYTHON retrogan_trainer_attractrepel_working_pytorch.py --iters $ITERS\
      "$CURR_DIR/oov_test_$PERCENTAGEREP/original.hdf" "$CURR_DIR/oov_test_$PERCENTAGEREP/arvecs.hdf" \
      "retrogan_$PERCENTAGEREP" "oov_test_$PERCENTAGEREP/retrogan_$PERCENTAGEREP/" --fp16 --cycle_mm false --epochs $EPOCHS
 }
function run05() {
    local PERCENTAGE=0.05
    local VISDEV=1
    CUDA_VISIBLE_DEVICES=$VISDEV generate_data_for_percentage && CUDA_VISIBLE_DEVICES=$VISDEV attractrepel_for_percentage && CUDA_VISIBLE_DEVICES=$VISDEV run_retro_gan_for_percentage && echo "Ran $PERCENTAGE">$PERCENTAGE.txt
}
function run10() {
