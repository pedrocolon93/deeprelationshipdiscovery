...
def create_model():
    ...
    merge_expand = tf.keras.layers.Dense(2048, activation='relu')(merge1)
    ...
    attention_expand = tf.keras.layers.Dense(2048, activation='relu')(merge_expand)
    ...
    semi_final_layer = tf.keras.layers.Dense(2048, activation='relu')(attention_expand)
    ...
    for rel in relations:
        ...
        weight = task_layer*out
        weight = tf.keras.layers.Dense(512,activation='relu')(weight)
        weight = tf.keras.layers.Dense(1,name=layer_name+"weight")(weight)
    ...
    def load_batch(output_name):
        ...
        for ndx in range(0, l, batch_size):
            try:
                ...
                x_1,x_2,y = zip(*c)
                print("Done")
                yield np.array(x_1), np.array(x_2), np.array(y),np.array(y_weight)
            except Exception as e:
                print(e)
                return False
        return True
        ...
        while True:
            for output in exclude:
                try:
                    ...
                    x_1, x_2, y,y_weight = it.__next__()
                    # print(x_1.shape)
