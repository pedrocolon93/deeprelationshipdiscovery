...
class DCGAN(object):
    ...
    def discriminator(self):
        ...
        self.D.add(Dense(512, input_dim=self.img_rows))
    ...
    def generator(self,input_dim=100):
        ...
        self.G.add(Dense(512, input_dim=input_dim))
        ...
        # self.G.add(Activation('sigmoiid'))
        self.G.add(Activation('tanh'))
        # self.G.compile(loss='mean_squared_error', optimizer=Adam(),
        #     metrics=['accuracy'])
        self.G.summary()
        return self.G

    def discriminator_model(self,lr=0.00001):
        if self.DM:
            return self.DM
    ...
    def adversarial_model(self,lr=0.000001):
        if self.AM:
            return self.AM
        # optimizer = RMSprop(lr=0.00001, decay=3e-8)
        # optimizer = RMSprop(lr=0.000001)

        # optimizer = Adam(lr=0.0000005,amsgrad=True)
        optimizer = Nadam(lr=lr)
        self.AM = Sequential()
        self.AM.add(self.generator(self.img_rows))
        self.AM.add(self.discriminator())
        self.AM.compile(loss='logcosh', optimizer=optimizer,\
            metrics=['accuracy'])
        return self.AM
...
class RETRO_DCGAN(object):
    def __init__(self):
        ...
        self.discriminator =  self.DCGAN.discriminator_model(lr=self.lr)
        self.adversarial = self.DCGAN.adversarial_model(lr = self.lr)
        self.generator = self.DCGAN.generator(self.img_rows)

    def train(self, train_steps=10000, batch_size=128, save_interval=0):
...
if __name__ == '__main__':
    ...
    print("In train")
    retrofit_dcgan.train(train_steps=200,save_interval=1000)
    ...
    pred_y = retrofit_dcgan.generator.predict(x)[0]

    resy = y-pred_y
    print(np.mean(resy))
    print(pred_y)

    # y_pred = retrofit_dcgan.generator.predict(common_vectors_test)
    # res = np.average(mean_squared_error(common_retro_vectors_test, y_pred))

    # scores = retrofit_dcgan.generator.test_on_batch(common_vectors_test, common_retro_vectors_test)
    # print(res)