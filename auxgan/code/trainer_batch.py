...
class Trainer(object):
    ...
    def __init__(self, src_emb, tgt_emb, mapping, discriminator, params):
        ...
        class IndexesDataset(Dataset):
            def __init__(self, amount):
                self.x = [x for x in range(0, amount)]
        ...
        ds = IndexesDataset(len(self.src_dico))
        t_ds = IndexesDataset(len(self.tgt_dico))
        self.sim_dl = DataLoader(ds, batch_size=self.params.batch_size,
                                 shuffle=True, num_workers=0, drop_last=True)
        self.sim_dl_iter = iter(self.sim_dl)

        self.dis_dl = DataLoader(ds, batch_size=self.params.batch_size,
                                 shuffle=True, num_workers=0, drop_last=True)
        self.dis_dl_iter = iter(self.dis_dl)

        self.t_dis_dl = DataLoader(t_ds, batch_size=self.params.batch_size,
                                   shuffle=True, num_workers=0, drop_last=True)
        self.t_dis_dl_iter = iter(self.t_dis_dl)
    ...
    def get_sim_xy(self):
        ...
        words = [self.src_dico[int(si)] for si in src_ids]
        tgt_ids = [self.tgt_dico.index(w) for w in words]
    ...
    def get_dis_xy(self, volatile):
        ...
        # select random word IDs
        bs = self.params.batch_size
        src_ids_original = torch.LongTensor(bs).random_(len(self.src_dico))
        # print(f'src_ids_original == {src_ids_original}')
        try:
            src_ids = torch.LongTensor(next(self.dis_dl_iter))
        except Exception as e:
            self.dis_dl_iter = iter(self.dis_dl)
            src_ids = next(self.dis_dl_iter)

        tgt_ids_original = torch.LongTensor(bs).random_(len(self.tgt_dico))
        # print(f'tgt_ids_original == {tgt_ids_original}')
        try:
            tgt_ids = torch.LongTensor(next(self.t_dis_dl_iter))
        except Exception as e:
            self.t_dis_dl_iter = iter(self.t_dis_dl)
            tgt_ids = next(self.t_dis_dl_iter)
    ...
    def heldoutall(self, params):
        ...
        for key in all_keys:
            ...
            hv = list(map(str, list(hv)))
            # hv = map(str, list(hv))
            # print(f'type(hv) == {type(hv)}')  # list
            # print(f'hv == {hv}')
            # print(f'type(key) == {type(key)}')  # str
            hv = " ".join([str(key)] + hv) + "\n"
            fhel.write(hv)
            if key in self.tgt_dico.word2id:
                av = ar_emb[self.tgt_dico.index(key)]
                av = av / np.linalg.norm(av)
                av = list(map(str, list(av)))
                # av = map(str, list(av))
                av = " ".join([str(key)] + av) + "\n"
                fall.write(av)
            else:
...
#     def forward(self, y_pred, y_true):
#         cost = 0.
#         for i in range(0, self.params.sim_neg):  # changed 'xrange' to 'range' -- ydx@20200904
#             new_true = torch.randperm(self.params.batch_size)
#             new_true = new_true.cuda() if self.params.cuda else new_true
#             new_true = y_true[new_true]
#             mg = self.params.sim_margin - F.cosine_similarity(y_true, y_pred) + F.cosine_similarity(new_true, y_pred)
...
#         # select random word IDs
#         bs = self.params.batch_size
#         src_ids = np.random.randint(0, len(self.src_dico), bs)  # YDX: replace with DataLoader
#         words = [self.src_dico[si] for si in src_ids]
#         tgt_ids = [self.tgt_dico.index(w) for w in words]
#         src_ids = torch.LongTensor(src_ids)
#         tgt_ids = torch.LongTensor(tgt_ids)
...
#         # select random word IDs
#         bs = self.params.batch_size
#         src_ids = torch.LongTensor(bs).random_(len(self.src_dico))  # YDX: replace with DataLoader
#         tgt_ids = torch.LongTensor(bs).random_(len(self.tgt_dico))  # YDX: replace with DataLoader
#         if self.params.cuda:
#             src_ids = src_ids.cuda()
#             tgt_ids = tgt_ids.cuda()
#
...
#         stats['SIM_COSTS'].append(loss.data.item())  # 1st try -- ydx@20200904
#
#         # check NaN
#         if (loss != loss).data.any():
#             logging.error("NaN detected (fool discriminator)")
...
#             stats['DIS_COSTS'].append(loss.data.item())  # 1st try -- ydx@20200904
#
#             # check NaN
#             if (loss != loss).data.any():
#                 logging.error("NaN detected (discriminator)")
...
#         stats['GEN_COSTS'].append(loss.data.item())  # 1st try -- ydx@20200904
#
#         # check NaN
#         if (loss != loss).data.any():
#             logging.error("NaN detected (fool discriminator)")
...
#             hv = hv / np.linalg.norm(hv)
#             #hv = map(str, list(hv))
#             hv = [str(x) for x in list(hv)]  # 2nd try -- ydx@20200905
#             hv = " ".join([str(key)] + hv) + "\n"
#             fhel.write(hv)
#             if key in self.tgt_dico.word2id:
#                 av = ar_emb[self.tgt_dico.index(key)]
#                 av = av / np.linalg.norm(av)
#                 #av = map(str, list(av))
#                 av = [str(x) for x in list(av)]  # 2nd try -- ydx@20200905
#                 av = " ".join([str(key)] + av) + "\n"
#                 fall.write(av)
#             else:
#                 fall.write(hv)
