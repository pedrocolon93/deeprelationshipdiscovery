...
class RetroCycleGAN():
    ...
    def compile_all(self, optimizer="sgd"):
        ...
        def create_opt(lr=0.1):
            if optimizer == "sgd":
                return tf.optimizers.SGD(lr=lr)
            elif optimizer == "adam":
    ...
    def train(self, epochs, dataset, batch_size=1, pretraining_epochs=150, rc=None):
        ...
        def train_(training_epochs, always_random=False):
            for epoch in range(training_epochs):
                ...
                if epoch == 59 or epoch == 99 or epoch ==134:
...
if __name__ == '__main__':
    ...
    learning_rates = np.linspace(1e-10, 1, num=150, endpoint=True)
    ds = {
        "original": "completefastext.txt.hdf",
        "retrofitted": "fullfasttext.hdf",
        "directory": "ft_full_paperdata/",
    ...
    for idx, lr in enumerate(learning_rates):
        ...
        rcgan = RetroCycleGAN(save_folder=save_folder, batch_size=32, generator_lr=lr, discriminator_lr=lr)
        ds_res = rcgan.train(pretraining_epochs=1, epochs=0, batch_size=32, dataset=ds, rc=ds["rc"])
    ...
    fig, ax = plt.subplots()
    ax.plot(t, s)
    ax.set(xlabel='learning rate', ylabel='simlex',
           title='Superconvergence plotting')
    ax.grid()
