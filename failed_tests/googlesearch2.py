...
class GoogleSearch:
    ...
    def search(self, query, num_results=10, prefetch_pages=True, prefetch_threads=10, language="en"):
        ...
        for i in range(pages):
            ...
            response = opener.open(GoogleSearch.SEARCH_URL + "?q=" + urllib.parse.quote(query) + "&hl=" + language + (
    ...
    def parseResults(self, results):
        searchResults = [];
        filtered = filter(lambda x: False if "class" in x.attrs.keys() else True, results)
