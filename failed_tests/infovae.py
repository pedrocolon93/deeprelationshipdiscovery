...
def encoder(x, z_dim):
    with tf.variable_scope('encoder'):
        ...
        fc3 = fc_lrelu(fc2,256)
        # conv1 = conv2d_lrelu(x, 64, 4, 2)
        # conv2 = conv2d_lrelu(conv1, 128, 4, 2)
        # conv2 = tf.reshape(conv2, [-1, np.prod(conv2.get_shape().as_list()[1:])])
        # fc1 = fc_lrelu(conv2, 1024)
        return tf.contrib.layers.fully_connected(fc3, z_dim, activation_fn=tf.identity)
...
def decoder(z, reuse=False):
    with tf.variable_scope('decoder') as vs:
        ...
        output = tf.contrib.layers.fully_connected(fc2,300)
        return output
...
X_train_iterator = tf.data.Dataset.from_tensor_slices(X_train).shuffle(buffer_size=10).batch(256).make_initializable_iterator()
X_test_iterator = tf.data.Dataset.from_tensor_slices(X_test).batch(2).make_initializable_iterator()
# plt.ion()

# Build the computation graph for training
z_dim = 20
input_vector_dimension = 300
# train_x = tf.placeholder(tf.float32, shape=[None, 28, 28, 1])
train_x = tf.placeholder(tf.float32, shape=[None,input_vector_dimension])
...
sess.run(X_test_iterator.initializer)
sess.run(X_train_iterator.initializer)

next_batch = X_train_iterator.get_next()
next_test_batch = X_test_iterator.get_next()
# Start training
for i in range(100000):
    ...
    if i % 2000 == 0:
        testbatch = sess.run(next_test_batch)
        print(testbatch)
        samples = sess.run(train_xr, feed_dict={train_x: testbatch})
        print(samples)
