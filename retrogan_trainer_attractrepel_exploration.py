...
class RetroCycleGAN():
    def __init__(self, save_index="0", save_folder="./", generator_size=32,
                 ...
                 lambda_cycle=0.4, lambda_id_weight=1,
        ...
        self.lambda_id = lambda_id_weight   # Identity loss
    ...
    def build_generator(self, name):
        ...
        d0 = dense(inpt, 2048, normalization=True,dropout=False)
        d0 = dense(d0, 2048, normalization=True,dropout=False)
        d0 = dense(d0, 2048, normalization=True,dropout=False)
                ...
                for batch_i, (imgs_A, imgs_B) in enumerate(load_batch(batch_size, always_random=always_random)):
                    ...
                    def named_logs(model, logs):
                        ...
                        for l in zip(model.metrics_names, logs):
                            result[l[0]] = l[1]
                        return result
                    logs1 = named_logs(self.combined, g_loss)
                    logs2 = {"mma":mm_a_loss,
                             "mmb":mm_b_loss}
                    logs1.update(logs2)
                    self.combined_callback.on_epoch_end(batch_i,logs1)
