...
if __name__ == '__main__':
    ...
    for idx, ds in enumerate(test_ds):
        print("Training")
        print(ds)
        rcgan = RetroCycleGAN(save_folder=save_folder, batch_size=64,discriminator_lr=0.0001,generator_lr=0.0001)
        models.append(rcgan)
        rcgan.load_weights("final","retrogan_glove_stable")
        sl = tools.test_sem(rcgan.g_AB, ds, dataset_location="testing/SimLex-999.txt",
                            fast_text_location="fasttext_model/cc.en.300.bin")[0]

        ds_res = rcgan.train(pretraining_epochs=200, epochs=0, batch_size=64, dataset=ds, rc=ds["rc"])
